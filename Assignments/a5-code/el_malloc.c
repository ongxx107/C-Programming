...
el_blockhead_t *el_block_below(el_blockhead_t *block){
  el_blockfoot_t *lowFt = PTR_MINUS_BYTES(block, sizeof(el_blockfoot_t) );
  if((void *) lowFt < (void*) el_ctl.heap_start){
      return NULL;
  }
  el_blockhead_t *lower = PTR_MINUS_BYTES(lowFt, lowFt->size + sizeof(el_blockhead_t));
...
el_blockhead_t *el_find_first_avail(size_t size){
  ...
  int length = el_ctl.avail->length;
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){  // example: new size = 60 and EL_BLOCk_OVERHEAD = 40
  int blockSize = block->size;
  if (blockSize < (new_size + EL_BLOCK_OVERHEAD)){
    return NULL;
  }
  else{
    el_blockfoot_t *oldFt = el_get_footer(block);
    oldFt -> size = blockSize - new_size - EL_BLOCK_OVERHEAD;
    block -> size = new_size;

    el_blockfoot_t *newFt = PTR_PLUS_BYTES(block, sizeof(el_blockhead_t) + new_size);
    el_blockhead_t *newHd = PTR_PLUS_BYTES(newFt, sizeof(el_blockfoot_t));

    newFt->size = block->size;
    newHd->size = oldFt->size;
...
// not the block header. Makes use of el_allocate_block() to find a
// suitable block. Returns NULL if no space is available.
void *el_malloc(size_t nbytes){
  el_blockhead_t *blockMem = el_find_first_avail(nbytes);
  if (blockMem != NULL){
    el_remove_block(el_ctl.avail, blockMem);

    el_blockhead_t *splitted1 = el_split_block(blockMem, nbytes);
    el_blockhead_t *splitted2 = el_block_above(splitted1);

    splitted1->state = 'u';
    splitted2->state = 'a';

    el_add_block_front(el_ctl.avail, splitted2);
    el_add_block_front(el_ctl.used, splitted1);

    void* ptr = PTR_PLUS_BYTES(splitted1, sizeof(el_blockhead_t));
    return ptr;
  }
  else{
    return NULL;
...
void el_merge_block_with_above(el_blockhead_t *lower){
  if ( lower == NULL || lower->state != 'a'
    || el_block_above(lower) == NULL || el_block_above(lower)-> state != 'a' ){
    return;
  }
  ...
  foot  -> size += higher->size + EL_BLOCK_OVERHEAD;
  el_get_footer(lower) -> size = foot -> size;       //new lower size
...
void el_free(void *ptr){
  ...
  if (head -> state == EL_USED){
    ...
    el_blockhead_t *higher= el_block_above(head);

    el_remove_block(el_ctl.used, head);
    head->state = 'a';
    el_add_block_front(el_ctl.avail, head);

    if ( (lower == NULL || lower->state != EL_AVAILABLE) && higher!=NULL && higher->state == EL_AVAILABLE ){
      el_merge_block_with_above(head);
    }
    else if ( (higher == NULL || higher->state != EL_AVAILABLE) && lower!=NULL && lower->state == EL_AVAILABLE ){
      el_merge_block_with_above(lower);
    }
    else if (lower != NULL && higher != NULL && lower->state == EL_AVAILABLE && higher->state == EL_AVAILABLE ){
      el_merge_block_with_above(head);
      el_merge_block_with_above(lower);
    }
    else{
